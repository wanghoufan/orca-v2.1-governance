...
# 预期差白名单（仅布局裸名）：AGENTS.md:36、scripts/orchestration/README.md:3。其余文件必须字节一致。
set -u
fail=0
check() { # $1=母版文件 $2=包文件
...
for pkg in "新项目模板包" "老项目迁移模板包"; do
  # AGENTS 白名单：仅允许 :36 裸名差 1 行（diff 输出恰 2 行 markers）
...
# 白名单外零容忍：上方仅报告“是否不同”；AGENTS:36 与 scripts README:3 的裸名差为预期，需人工确认仅此两处：
echo "--- 预期差确认（应仅 2 处） ---"
