...
> 分发注记（2026-09-11，随 ORCA V2.1 治理模板分发版拷入包根，原件在业务仓 `Infrastructure/orca-deepseek-bridge/docs/`，两边内容一致；Contract 更新时以业务仓为准重拷）。

## 0. V2.1 Compatibility Verdict

**PASS**（2026-09-10，生产接入收口实测）。

仍需治理模板配合的最小接口（共 3 项，**当前均待落地**——2026-09-11 复核：此前写入的 ①② 已被模板侧改动还原）：

1. ⏳ 待落地：`USER_MODEL_OVERRIDE.md` 规则行——builder 槽位写精确 ID `deepseek-bridge/deepseek-v4-flash` 即启用本通道，仅用户改表/口头指定时生效。
2. ⏳ 待落地：`编排者提示词.md` 派发口一句——已正式接入的外部 Builder Runtime / Bridge 由派工基础设施自动调用，不算"起终端"；TM 不人工开终端/复制 Prompt/QA/Reviewer/Session ID/历史上下文；resume 由派工基础设施维护。
...
- 逻辑 route/model ID：`deepseek-bridge/deepseek-v4-flash`（builder 插槽专用；禁别名）。
...
- 账本 `model` 字段写 `deepseek-bridge/deepseek-v4-flash`（实际精确 ID，不写虚构别名）；`rework` 按 V2.1 定义 = supervisor 打回次数，QA round 数不计入。
...
1. override 表 builder 行写 `deepseek-bridge/deepseek-v4-flash` 即选择本通道（用户决定权）。
