# ORCA V2.1 治理体系说明（对外版，2026-09-22）
...
- 只判四类：CHANGE_CLASS（A/B/C）、ISSUE_OWNER（6角色）、USER_REQUIRED（5枚举）、P0_HARD（bool＋风险）。
- 只做建议（advisory），规则冲突听规则，失败回原逻辑；Human Gate/删除/不可逆/付费/换模型永不自动批。
- 现态（2026-09-22）：TypeSafe 直调 10/10 实测通过，失败自动回退原逻辑。
...
## 五、模型与分工（T3 现势，11 行；下表用精确 ID，task-manager 行开窗口时定除外）

| 角色 | 模型（精确ID） | 通道 |
|---|---|---|
| task-manager | 开窗口时定 | 本窗口直派 |
| supervisor | opencode-go/muse-spark-1.3-contributor | opencode 直调 |
| builder | codebuddy/deepseek-v4.1-flash（限额→codebuddy/glm-5.3-flash→停） | codebuddy |
...
另：本机 opencode 另接火山方舟 Coding Plan（`volcengine-plan/ark-code-latest`，
控制台切模型，reasoningEffort high），暂未进分工表，属个人实验通道。
...
| 模型分工真相源 | `USER_MODEL_OVERRIDE.md` | 11 行精确ID；改表必真调；T3 现势（T1/T2 快照在 `temp/`，回退由用户口头指定编号、按改表规则执行） |
