# 收编说明（V2.1 体系，2026-09-12 用户拍板）

> 本协议收编进 ORCA V2.1 分发版，**仅作为防停摆机制规范**采用：三层监督（L1 回合内 rolling wait / L2 文件状态机+STATE.md / L3 外部 launchd watchdog）、回合结束检查单（§二十九）、后台进程纪律（§三十一）、验证后再声称（§三十二）、停摆根因分类 R1-R5（§三十三）。
> **冲突隔离**：§一"禁止预设固定角色/流程动态识别"与 V2.1 十卡制（9+1 固定）冲突，**不采用**——角色一律以 V2.1 `docs/roles/` 十卡为准。
