...
- DeepSeek V4.1 Flash 双通道备注（2026-09-11用户定，勿混勿删，留测待切）：A=DeepSeek Bridge通道（上条，精确ID `deepseek-flash`，Runtime `deepseek-bridge`，已测过）；B=PI智能体通道（codebuddy，模型列 `deepseek-v4.1-flash`，档位high写口头不进模型列，Runtime已填codebuddy）。A/B同模型不同通道，表内混写一律以本条为准拆分；两者都保留，后续可切换；测试只测B（A已测过），B只读探针 `codebuddy --model deepseek-v4.1-flash --effort high --help` 已rc=0，单发pong PASS（BUGS 09-11），批量压测待批；主备归属已定（当前主用B，A保留可切回）。
