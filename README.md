...
- `prompts/`：编排者/外部开发者/迁移整理三份提示词＋《Orca 通用编排者持续推进协议》（防停摆三层监督收编版；其动态角色论与十卡制冲突，不采用）
- `history/`：重塑说明、更新说明（历史，看现行先看根）＋审查报告3份（-y轮/现势/逐派，结论已落实部分见现行文件）
- `templates/`：归位表模板
...
- `老项目迁移模板包/`：老项目迁移所需的完整文件包，包含 `【迁移整理】` 提示词。
